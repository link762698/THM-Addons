...
public class OffhandManager extends Module {
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        if (Modules.get().get(ScaffoldTHM.class).hasWorked()) return;
    ...
    private boolean shouldEat() {
        return (mc.player.getHungerManager().getFoodLevel() <= hungerThreshold.get() || autoFireRes.get() && !mc.player.getActiveStatusEffects().containsKey(StatusEffects.FIRE_RESISTANCE)) && !Modules.get().get(ScaffoldTHM.class).hasWorked();
    }
